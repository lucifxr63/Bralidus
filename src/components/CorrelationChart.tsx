...
export type MacroSeriesId = 'GDP' | 'CPIAUCSL' | 'FEDFUNDS';
...
interface Props {
  hypothesis: 'H1' | 'H2' | 'H3';
  macroTitle: string;           // document_title en knowledge_nodes
  macroLabel: string;           // etiqueta eje izquierdo
  macroUnit: string;            // unidad para tooltip
  pymeMetric: PymeMetric;
  pymeLabel: string;            // etiqueta eje derecho
  pymeUnit: string;
  companyRut?: string;          // si está vacío → solo serie macro
...
const MACRO_COLOR = '#0EB5C6';   // teal — serie macro (línea sólida)
const PYME_COLOR  = '#F59E0B';   // amber — serie PYME (línea punteada)

// ── Normalización min-max [0, 100] para overlay visual ────────────────────────
...
export default function CorrelationChart({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // ── 1. Datos macro desde knowledge_nodes ──────────────────────────────
      ...
      // ── 2. Datos PYME desde pyme_financials (opcional) ────────────────────
      ...
      {loading ? (
        ...
        <ResponsiveContainer width="100%" height={200}>
          <ComposedChart data={data} margin={{ top: 4, right: 16, left: -20, bottom: 0 }}>
            ...
            {/* Línea punteada: PYME — solo si hay datos */}
            {hasPyme && (
              <Line
                yAxisId="normalized"
                type="monotone"
